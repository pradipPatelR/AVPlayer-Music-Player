...
@objc public protocol CachingPlayerItemDelegate {
    ...
    /// Called after initial, means we are unknown.
    @objc optional func playerItemUnknown(_ playerItem: CachingPlayerItem)
...
public final class CachingPlayerItem: AVPlayerItem {
    ...
    public init(url: URL, saveFilePath: String, customFileExtension: String?, avUrlAssetOptions: [String: Any]? = nil) {
        ...
        if let ext = customFileExtension {
            ...
            fatalError("CachingPlayerItem error: url pathExtension empty, pass the extension in `customFileExtension` parameter")
        }
    ...
    public override func observeValue(forKeyPath keyPath: String?,
        ...
        case .failed:
            ...
            debugPrint("CachingPlayerItem status: failed with error: \(String(describing: error))")
            DispatchQueue.main.async { self.delegate?.playerItemDidFailToPlay?(self, withError: self.error) }
        case .unknown:
            // Player item is not yet ready.
            debugPrint("CachingPlayerItem status: uknown with error: \(String(describing: error))")
            DispatchQueue.main.async { self.delegate?.playerItemUnknown?(self) }
